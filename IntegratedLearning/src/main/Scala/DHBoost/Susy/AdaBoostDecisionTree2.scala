...
object AdaBoostDecisionTree2 {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val spark: SparkSession = SparkSession
            .builder
            .appName("AdaBoostDecisionTree_Susy")
            .getOrCreate()

        // 写入配置
        val path = "hdfs://172.31.238.20:8020/user/chandler/recurrence/susy/AdaBoostDecisionTree2/"
        log.append("job,AdaBoostDecisionTree_Susy\n")

        // 加载数据，并进行类型转换
        val dataFrame = spark
            .read.format("csv")
            .load("hdfs://172.31.238.20:8020/user/chandler/recurrence/susy/train/repartationRdd/part-00002*.csv")// 加载第2个分区
