...
object AdaBoostSVM {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val spark: SparkSession = SparkSession
            .builder
            .appName("AdaBoostSVM_Higges")
            .getOrCreate()

        // 写入配置
        val path = "hdfs://172.31.238.20:8020/user/chandler/recurrence/higges/AdaBoostSVM/"
        log.append("job,AdaBoostSVM_Higges\n")

        // 加载数据，并进行类型转换
        val dataFrame = spark
            .read.format("csv")
            .load("hdfs://172.31.238.20:8020/user/chandler/recurrence/higges/train/repartationRdd/part-00002*.csv")// 加载第2个分区
        ...
        val modelArray: Array[LinearSVCModel] = new Array[LinearSVCModel](maxIter)
